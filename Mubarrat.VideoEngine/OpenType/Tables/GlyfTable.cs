...
public sealed class GlyfTable : IOpenTypeTable
{
    ...
    public void Parse(ParsedTables tables, OpenTypeReader.TableScope scope) => tables.Request<MaxpTable, LocaTable>((maxp, loca, scope) =>
    {
        ...
        resolvedPoints = new Point[maxp.NumGlyphs][];
    ...
    public Point[] GetPoints(int glyphIndex)
        => ResolvePoints(glyphIndex);

    public Point GetPoint(int glyphIndex, int pointIndex)
    ...
    private static CompositeGlyphDescription ReadCompositeGlyph(OpenTypeReader reader)
    {
        ...
        do
        {
            ...
            {
                transform = Matrix2D.Scale(reader.ReadF2Dot14(), reader.ReadF2Dot14());
            }
    ...
    private Point[] ResolvePoints(int glyphIndex)
    {
        ...
        {
            points = new Point[sgd.XCoordinates.Length];
            for (int i = 0; i < points.Length; i++)
                points[i] = new Point(sgd.XCoordinates[i], sgd.YCoordinates[i]);
        }
    ...
    private Point[] ResolveCompositePoints(CompositeGlyphDescription composite)
    {
        int capacity = 0;
        ...
        for (int i = 0; i < composite.Components.Length; i++)
        {
            ...
            else
            {
                ...
                Point parentPoint = points[comp.Arg1];
                Point childPoint = childPoints[comp.Arg2] * comp.Transform;
                offset = parentPoint - childPoint;
            }

            for (int p = 0; p < childPoints.Length; p++)
                points.Add((childPoints[p] * comp.Transform) + offset);
        }
