...
/// <summary>
/// Provides a video encoder that uses Intel Quick Sync Video (QSV) to encode BGRA frames as H.264 video in NV12 pixel
/// format.
/// </summary>
/// <remarks>This encoder is optimized for hardware-accelerated H.264 encoding using the QSV backend and expects
/// input frames in BGRA format. The encoded output is written to the specified file path in H.264 format. Thread safety
/// is not guaranteed; use separate instances for concurrent encoding operations.</remarks>
public unsafe class H264QsvNv12Encoder : VideoEncoder
{
    public static readonly EncoderConstructor Constructor = (w, h, fps, path) => new H264QsvNv12Encoder(w, h, fps, path);

    private readonly AVFrame* frameBgra;
    private readonly AVFrame* frameNv12;
    private readonly SwsContext* swsCtx;

    public H264QsvNv12Encoder(ushort width, ushort height, ushort fps, string outputPath)
        : base(width, height, fps, outputPath, "h264_qsv", AVPixelFormat.AV_PIX_FMT_NV12)
    {
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        // Free unmanaged FFmpeg resources
        ...
        base.Dispose(disposing); // Dispose base unmanaged resources
    }
}
